...
int main(int argv, char ** argc) {
    ...
    if (argv < 2) {
        printf("Usage: %s <input_file_name> [--hd] [--print]\n", argc[0]);
        return 1;
    }
    if (argv >= 3) {
        if (strcmp(argc[2], "--hd") == 0)
            MODE = HD;
        if (strcmp(argc[2], "--print") == 0)
            PRINT = 1;
    }
    if (argv >= 4) {
...
//source_info.st_size = get_frame_size() * 15; // HACK HACK HACK HACK HACK HACK **** DANGER ****
    ...
    double goodness_factor[NUM_SYMBOLS] = {0};
    unsigned long total_occurances = get_num_frames(&source_info) * get_frame_size();
    goodness_factor[0] = 1e6;
    for (int i = 1; i < NUM_SYMBOLS; i++)
        goodness_factor[i] = -1e6;
    for (int i = 0; i < NUM_SYMBOLS; i++) {
    //    goodness_factor[i] = (double)occurances[i] / (double)total_occurances - .3;
        printf("Base goodness: %f\n", goodness_factor[i]);
    }
    ...
    for (int curr_diff = 0; curr_diff < get_num_frames(&source_info) - 1; curr_diff++) {
        ...
        double goodness = 0;
        for (int i = 0; i < NUM_SYMBOLS; i++)
            goodness += goodness_factor[i] * abs(occurances[i] - base_occurances[i]);//compute_abs_diff(occurances, base_occurances, i);
            //goodness += compute_abs_diff(occurances, base_occurances, i);
        // Save/make space for some statistical-tracking variables
        unsigned int best_prior_diff = curr_diff;
        double orig_goodness = goodness;
        double trailing_goodness = -1; // Underflow to max of range
        ...
        for (int backtest_fdiff_idx = 0; backtest_fdiff_idx < curr_diff; backtest_fdiff_idx++) {
            ...
            double local_goodness = 0;
            for (int i = 0; i < NUM_SYMBOLS; i++)
                //local_goodness += compute_abs_diff(occurances, local_occurances, i);
                local_goodness += goodness_factor[i] * abs(occurances[i] - local_occurances[i]);//compute_abs_diff(occurances, base_occurances, i);
            if (local_goodness > goodness) {
                best_prior_diff = backtest_fdiff_idx;
                goodness = local_goodness;
            }
            //printf("Using diff %d as a basis for diff %d would have mismatch count %lu\n", backtest_fdiff_idx, curr_diff, local_goodness);
            // Update stats
            if (local_goodness > trailing_goodness)
                trailing_goodness = local_goodness;
        }
        // Record the residual and basis index or the original diff
        if (goodness != orig_goodness) {
            for (int p = 0; p < get_frame_size(); p++)
                better_frame_diffs[curr_diff][p] = frame_diffs[curr_diff][p] - better_frame_diffs[best_prior_diff][p];
        ...
        if (PRINT) {
            if (goodness == orig_goodness)
                printf("No more efficient diff available for diff %d (best competing is %f vs %f)\n", curr_diff, trailing_goodness, orig_goodness);
            else
                printf("Most efficient prior diff for diff %d is diff %d at goodness %f vs original goodness %f\n", curr_diff, best_prior_diff, goodness, orig_goodness);
        }
    }
    ...
    */
    ...
    for (int curr_diff = 0; curr_diff < get_num_frames(&source_info) - 1; curr_diff++) {
        ...
        for (int pixel = 0; pixel < get_frame_size(); pixel++)
            best_cost += codes[frame_diffs[curr_diff][pixel]].code_length;
        // Save/make space for some statistical-tracking variables
        ...
        for (int backtest_fdiff_idx = 0; backtest_fdiff_idx < curr_diff; backtest_fdiff_idx++) {
            ...
            for (int pixel = 0; pixel < get_frame_size(); pixel++) {
                uint8_t diff = frame_diffs[curr_diff][pixel] - frame_diffs[backtest_fdiff_idx][pixel];
    ...
    for (int frame = 0; frame < get_num_frames(&source_info) - 1; frame++) {
        for (int pixel = 0; pixel < get_frame_size(); pixel++) {
            ...
            buffer_len += pair->code_length % 8; // Extend buffer with residual length
            // If buffer is over a byte, flush it
            if (buffer_len >= 8) {
